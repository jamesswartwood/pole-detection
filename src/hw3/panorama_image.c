...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        float smallest = l1_distance(a[j].data, b[0].data, a[j].n);
        for (i = 1; i < bn; i++) {
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; i++) {
        ...
        matrix homography = compute_homography(m, e);
        int sample = model_inliers(homography, m, n, thresh);
        if (best < sample) {
            free_matrix(Hb);
            Hb = compute_homography(m, sample);
            best = sample;
            if (cutoff < best) {
                return Hb;
            }
    ...
    for (int z = 0; z < a.c; z++) {
        for (int y = topleft.y; y < botright.y; y++) {
            for (int x = topleft.x; x < botright.x; x++) {
                point projected = project_point(H, make_point(x, y));
                if (projected.x >= 0 && projected.x < b.w &&
                    projected.y >= 0 && projected.y < b.h) {
                    set_pixel(c, x - dx, y - dy, z,
                        bilinear_interpolate(b, projected.x, projected.y, z));
                }
            }
        }
    }
